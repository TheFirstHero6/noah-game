...
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'Resource') THEN
        ALTER TABLE "Resource" ADD COLUMN IF NOT EXISTS "realmId" TEXT;
    END IF;
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'City') THEN
        ALTER TABLE "City" ADD COLUMN IF NOT EXISTS "realmId" TEXT;
    END IF;
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'Army') THEN
...
BEGIN
    ...
    -- If no users exist, skip the data migration (shadow database)
    IF default_owner_id IS NULL THEN
        RETURN;
    END IF;

    -- Check if "Imperium Fragmentum" realm already exists
    IF NOT EXISTS (SELECT 1 FROM "Realm" WHERE "name" = 'Imperium Fragmentum') THEN
    ...
    ELSE
        ...
        SELECT "id" INTO default_realm_id FROM "Realm" WHERE "name" = 'Imperium Fragmentum' LIMIT 1;
    END IF;
    ...
    -- Now make realmId columns required (only if tables exist)
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'Resource') THEN
...
-- Conditionally create indexes and constraints for existing tables
DO $$
BEGIN
    -- Create unique index on Resource if table exists
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'Resource') THEN
        ...
        -- Add foreign key if realmId column exists and is not null
        IF EXISTS (SELECT FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'Resource' AND column_name = 'realmId') THEN
            BEGIN
    ...
    -- Add foreign key for City if table exists
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'City') THEN
        IF EXISTS (SELECT FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'City' AND column_name = 'realmId') THEN
            BEGIN
    ...
    -- Add foreign key for Army if table exists
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'Army') THEN
        IF EXISTS (SELECT FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'Army' AND column_name = 'realmId') THEN
            BEGIN
